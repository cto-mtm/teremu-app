 ...
 * on the restaurant doc as `plan: "free" | "pro"`; scan usage meters on
 * the same doc (`scanPeriod` = YYYY-MM, `scanCount`), reset lazily when
 * the month changes.
 *
...
export async function getPlanInfo(rid: string): Promise<PlanInfo> {
  ...
  const plan: Plan = snap.get("plan") === "pro" ? "pro" : "free";
