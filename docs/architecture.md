...
1. **Scan**: an on-device quality gate (brightness + variance-of-Laplacian blur check, ~5ms) can hold a bad capture and ask retake/use-anyway. Good captures POST to `POST /invoices`; the function stores the image and creates an invoice with `status: "processing"` — the camera never blocks. Uploads are **idempotent by content hash**: each page's SHA-256 lands in `imageHashes`, a byte-identical re-upload (double-tap, client retry) is a 409 pointing at the original — checked *before* the quota, so duplicates never burn a scan — and re-adding the same page to a multi-page capture refuses likewise. Exact match only: two camera shots of the same paper are different bytes and legitimately two scans.
2. **OCR (staged)**: the Storage trigger sends the image to a vision model (provider-agnostic — `docs/llm.md`), which first *classifies* (`kind: receipt | other` + confidence — non-documents fail distinctly as `not_a_document`) then *extracts* line items. The prompt carries the restaurant's own name as the BUYER so the "cliente / bill to" box is never read as the vendor, with a deterministic backstop in `ocr.ts` that nulls a vendor exactly matching the restaurant's normalized name. A deterministic *arithmetic validation* stage cross-checks qty × price against line totals and the line sum against the printed total; discrepancies become `warnings` on the invoice and per-line flags rather than failures. Status becomes `needs_review`.
...
**Billed vs real time.** Facturas are the accounting truth, but consolidated billing means albaranes can arrive for weeks before their factura does — so the Dashboard has a *Facturado / Tiempo real* toggle (shown only while pending notes exist). `realtimeSpend` adds every approved delivery note that reconciliation reports as unmatched (and not hand-marked handled), shaped as an invoice so all existing aggregates consume it unchanged; unpriced albarán lines are valued at the matched ingredient's current price, with unpriceable lines surfaced honestly. When the factura arrives and reconciles, its notes drop out — the views converge and nothing is double-counted. The "Recibido sin facturar" banner shows the delta in both views. Price history and alerts stay billed-only: albarán valuations are estimates, not payments.
